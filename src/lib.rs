...
use settings::{Base, EncodeMode, ReadMode};
...
fn handle_base_type(settings: &mut Settings,
                    base_type: Option<String>) -> Result<(), &'static str> {
    match base_type {
        Some(base_type) => {
            match &base_type[..] {
...
fn handle_input(settings: &mut Settings, value: &str) {
    match settings.read_mode() {
        ReadMode::FileName => { settings.add_file(value); }
        ReadMode::StdIn => { settings.add_string(value); }
    }
}
